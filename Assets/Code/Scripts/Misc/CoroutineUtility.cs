...
public static class CoroutineUtility
{
    ...
    private static Dictionary<float, WaitForSeconds> _cachedWaitDictionary = new Dictionary<float, WaitForSeconds>();
    ...
    private static Dictionary<float, WaitForSecondsRealtime> _cachedWaitRealtimeDictionary = new Dictionary<float, WaitForSecondsRealtime>();
    #endregion
    ...
    {
        return script.StartTimer(onComplete, new WaitUntil(predicate));
    }
    /// <summary>
    ...
    {
        return script.StartTimer(onComplete, new WaitWhile(predicate));
    }
    #endregion

    #region Other Stuff
    public static IEnumerator CancellableWaitForSeconds(float time, bool unscaled, Func<bool> cancelPredicate)
    {
        while (time > 0)
        {
            time -= unscaled ? Time.unscaledDeltaTime : Time.deltaTime;

            if ((unscaled || Time.timeScale > 0) && cancelPredicate.Invoke())
            {
                yield break;
            }
